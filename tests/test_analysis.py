...
def test_degenerate_cell_cannot_manufacture_a_huge_t():
    ...
    assert abs(c.t_clustered_unguarded) > 15
    assert abs(c.t_clustered) < 2.0
    assert c.se_governing == c.se_binomial
    assert c.se_binomial > 10 * c.se_naive


def test_fresh_subset_filters_on_staleness_per_horizon():
    snaps = [
        make_snapshot("a", "e1", "T-1h", 50.0, 1, staleness=100),
        make_snapshot("b", "e2", "T-1h", 50.0, 1, staleness=7200),
        make_snapshot("c", "e3", "T-7d", 50.0, 1, staleness=86400),
    ]
    df = analysis.snapshots_to_frame(snaps)
